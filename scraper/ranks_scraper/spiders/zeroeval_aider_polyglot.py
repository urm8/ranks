"""ZeroEval aider-polyglot — dedicated in-spider parser for ALL entries."""
...
class ZeroevalAiderPolyglotSpider(SourceSpider):
    name = "zeroeval_aider_polyglot"
    source_title = 'ZeroEval Aider Polyglot'
    start_urls = ['https://api.zeroeval.com/leaderboard/benchmarks/aider-polyglot?top_n=500']
    kind = 'scores'
    engine = 'http'

    def parse_zeroeval_aider_polyglot_entries(self, body: str) -> list[dict]:
        try:
    ...
    def parse_response(self, response):
        rankings = self.parse_zeroeval_aider_polyglot_entries(response.text)
        models = {
            row["model_id"]: {
                "name": row["model"],
...
if __name__ == "__main__":
    ...
    sample = Path(__file__).with_name('zeroeval_aider_polyglot_sample.json')
    body = sample.read_text() if sample.exists() else ""
    spider = ZeroevalAiderPolyglotSpider()
